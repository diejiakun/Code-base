...
int main(int argc, char const *argv[])
{
    ...
    // 打印16进制IP地址作为对照
    printf("192.168.190.131 的16进制表示为 0x%X 0x%X 0x%X 0x%X\n", 192, 168, 6, 101);

    // 不推荐使用,因为输入-1返回的地址是一个有效地址（255.255.255.255）
    server_in_addr_t = inet_addr("192.168.190.131");
    printf("inet_addr convert: 0x%X\n", server_in_addr_t);

    inet_aton("192.168.190.131", &server_in_addr);
    printf("inet_aton convert: 0x%X\n", server_in_addr.s_addr);

    // 推荐使用
    // 字符串转sockin_addr结构体
    inet_pton(AF_INET, "192.168.190.131", &server_addr.sin_addr);
